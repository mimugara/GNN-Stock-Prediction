...
class Alphas:
    ...
    def cal_alphas(self, underlying, n_jobs: int, alpha_names=None,):
        os.makedirs("factor_data", exist_ok=True)
        ...
        def _cal_alphas(uly, m):
            ...
            res.to_hdf("factor_data/{}_{}.h5".format(self.name, m), key=m)
    ...
    def tst_alphas(self, quantiles=5, alpha_names=None, measure="open"):
        ...
        for m in alpha_names:
            ...
            fac = pd.read_hdf("factor_data/{}_{}.h5".format(self.name, m), key=m).iloc[1215:]
            fac.index = pd.to_datetime(fac.index)
            close = pd.read_hdf("Ashare_data/1day_data/pv.h5", key=measure).iloc[1215:]
